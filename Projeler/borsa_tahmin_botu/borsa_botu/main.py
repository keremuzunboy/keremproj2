...
def prepare_data(df):
    ...
    features = ['Open', 'High', 'Low', 'Close', 'Volume', 'SMA_20', 'SMA_50', 'RSI', 'MACD', 'Signal']
    X = df[features]
    y = df['Target']
...
def plot_results(df, predictions):
    ...
    plt.title('Borsa Tahmin Sonuçları')
    plt.xlabel('Tarih')
    plt.ylabel('Fiyat')
    plt.legend()
    plt.show()
...
if __name__ == "__main__":
    ticker = "ASELS.IS"  # ASELSAN örneği
    ...
    last_data = df.iloc[-1][['Open', 'High', 'Low', 'Close', 'Volume', 'SMA_20', 'SMA_50', 'RSI', 'MACD', 'Signal']].values
    next_day_prediction = predict_next_day(model, scaler, last_data)
    print(f"Gelecek gün için tahmin edilen kapanış fiyatı: {next_day_prediction:.2f}")